...
def xush_kelibsiz_xabari(foydalanuvchi_obj):
    """
    (matn, entities, parse_mode) tuple qaytaradi.
    Username bo'lsa @mention, bo'lmasa TEXT_MENTION entity.
    """
    from telegram import MessageEntity

    ism   = foydalanuvchi_obj.full_name
    uname = foydalanuvchi_obj.username

    suffix = (
        "!\n\n"
        "Siz <b>Avtotest Online Kursiga</b> qo'shildingiz 🎉\n\n"
        "📅 Kurs davomida <b>14 kun</b> sizga yordam berishga tayyormiz!\n\n"
        "❓ Savol yoki muammo bo'lsa:\n"
        "  • Guruhda yozing — hamkorlar yordam beradi\n"
        f"  • Adminga murojaat qiling: {ADMIN_USERNAME}\n\n"
        "🚗 Muvaffaqiyatli o'qishlar!"
    )

    if uname:
        matn = f"👋 Xush kelibsiz, @{uname}{suffix}"
        return matn, None, "HTML"
    else:
        prefiks = "👋 Xush kelibsiz, "
        matn = prefiks + ism + suffix
        offset = len(prefiks.encode("utf-16-le")) // 2
        length = len(ism.encode("utf-16-le")) // 2
        entity = MessageEntity(
            type=MessageEntity.TEXT_MENTION,
            offset=offset,
            length=length,
            user=foydalanuvchi_obj,
        )
        return matn, [entity], None
...
async def yangi_avo(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if eski_holat in (ChatMember.LEFT, ChatMember.BANNED) and \
       yangi_holat in (ChatMember.MEMBER, ChatMember.ADMINISTRATOR):
        ...
        try:
            matn, entities, parse_mode = xush_kelibsiz_xabari(foydalanuvchi)
